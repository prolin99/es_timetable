 ...
 <h3><{$data.n_y}> 學年度<{$data.n_s}> 學期   <{$data.class_list_c[$data.my_class_id]}>功課表 </h3>
 	...
 	<a class="btn btn-info" href="export.php?class_id=<{$data.my_class_id}>">下載課表</a>
      ...
      <{foreach  from=$cell_tab  key=w item=cell_data  }>
          ...
          <{if ($cell_data.self_lock or( ($cell_data.class_id) and ( $cell_data.class_id <> $data.my_class_id)) ) }>  <{*有課且不同班*}>            <{assign var="cell_showed"  value=1 }>
            <div class="nobox" data_ref="sect_<{$smarty.section.di.index}>_<{$smarty.section.si.index}>" id="sect_<{$smarty.section.di.index}>_<{$smarty.section.si.index}>" style="background:#AAA;" title='課表鎖定，不可排入'>
            </div>
              <{if $w==1}>單週-<{/if}>
              <{if $w==2}>雙週-<{/if}>
